    ...
    (() => {
        ...
        const switchTypes = () => {
            document.addEventListener("DOMContentLoaded", (function() {
                const themeItems = document.querySelectorAll(".options-switch__theme-item");
                const switchButtons = document.querySelectorAll(".header__switcher-buttons .switch-button");
                const divider = document.querySelector(".header__switcher .divider");
                const themeBlocks = document.querySelectorAll(".theme-block");
                themeItems.forEach((item => {
                    item.addEventListener("click", (() => {
                        const themeType = item.getAttribute("data-type-theme");
                        handleThemeSwitch(themeType);
                    }));
                }));
                switchButtons.forEach((switchButton => {
                    switchButton.addEventListener("click", (() => {
                        const themeType = switchButton.getAttribute("data-type-theme");
                        handleThemeSwitch(themeType);
                    }));
                }));
                function handleThemeSwitch(themeType) {
                    themeItems.forEach((el => {
                        el.classList.remove("active");
                    }));
                    switchButtons.forEach((button => {
                        const buttonThemeType = button.getAttribute("data-type-theme");
                        const shouldAddClass = buttonThemeType === themeType;
                        button.classList.toggle("active", shouldAddClass);
                    }));
                    const matchingThemeItem = document.querySelector(`.options-switch__theme-item[data-type-theme="${themeType}"]`);
                    if (matchingThemeItem) matchingThemeItem.classList.add("active");
                    updateDividerPosition(themeType);
                    themeBlocks.forEach((block => {
                        const blockThemeType = block.getAttribute("data-theme");
                        const shouldAddClass = blockThemeType === themeType;
                        block.classList.toggle("active", shouldAddClass);
                    }));
